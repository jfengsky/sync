			...
			*/	
			_getTip:function () {
				...
				var objOptions =  {
					...
					loadingImg:"loading...",
					maxWidth:parseInt(css.maxWidth,10),
					minWidth:parseInt(css.minWidth,10),
					content:classNames.tipContent//$.tmpl.render(this.temp, this.data)
				}
				
				div.innerHTML = $.tmpl.render(options.templs.tipTempl,objOptions);
	...
	jmp.defaults = {
		options : {
			...
			templs:{
				...
				"tipTempl":'<div id=${id} class=${tip}  group=${group} style="visibility:hidden;display:block;z-index:99;margin:0;left:-9999px;top:-9999px;position:absolute;max-width:${maxWidth}px;min-width:${minWidth}px;_width:${maxWidth}px;"><div class="${box} ${boxType} ${boxArrow}"><b class="${arrow}"></b><div class="${loading}">${loadingImg}</div><div class=${content}></div></div></div>',
				"jmp_table":'${txt}',
