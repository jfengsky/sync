...
define(function (require, exports, module) {
  ...
  function ChangeArea(){
    ...
    this._firstGetIds = function(){
      var firstIds = [];
      $.each($('#J_main li'), function(index, item){
        if($(item).attr('id')){
          firstIds.push($(item).attr('id').slice(2) - 0)
        }
      });
      return firstIds;
    };
    ...
    this.init = function(){
      ...
      $.each($('#J_nav_list li'), function(index, item){
        if($(item).hasClass('current')){
          defaultCityId = $(item).find('a').attr('data-id');
        }
      });

      // 每隔2分钟请求一次数据
      // upStoke.clear();
      // upStoke.init(itemId.join(','), _cityId);
      // 便利li 获取当前商品id
      upStoke.init(self._firstGetIds().join(','), defaultCityId);

      // 导航切换
      $('#J_nav_list').undelegate('li');
      $('#J_nav_list').delegate('li', 'click', function(){
        self._clearnav(this);
      });

      // 更多切换
      $('#J_nav_list').undelegate('.city_item');
       ...
       * TODO ipad 操作优化
       */
      $('#J_nav_list li.more_city').unbind();
      $('#J_nav_list li.more_city').bind('mouseenter mouseleave', function(ev){
