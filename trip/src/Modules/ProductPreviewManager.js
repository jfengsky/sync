    ...
    var PreviewList = inherit({
		...
		*/
        __constructor: function (options) {
            ...
            // 目前循环播放等只用到图片，视频直接跳过
            //根据传来的数据，长度是图片加视频的长度，现在减去视频个数
            var videoOptions = 0;
            if ($("a[data-type='video']").length) {
                videoOptions = $("a[data-type='video']").length;
            }
            this.countTotalUsage = (this.options.countImage - videoOptions); //this.countPreview
		...
		*/
        __constructor: function (options) {
            ...
            if (bIsIpad) {
                var touch_startX = 0,
					touch_endX = 0,
					touch_event = document.getElementById('js-preview-photo');
                touch_event.addEventListener('touchstart', function (e) {
                    touch_startX = event.touches[0].clientX;
                });
                touch_event.addEventListener('touchend', function (e) {
                    touch_endX = event.changedTouches[0].clientX;
                    if (touch_startX > touch_endX) {
                        $('#js-preview-photo').find('.next').click();
                    } else if (touch_startX < touch_endX) {
                        $('#js-preview-photo').find('.prev').click();
                    }
                });
            }
            this.options = _.extend({}, options);
        ...
        onListPreviewChange: function (index, previewType, changeType) {
            ...
            if (previewType == PREVIEW_TYPE.IMAGE) {
                ...
                this.pic.show()
					.changePic({
					    nowIndex: index,
					    Url: $(data).find('img').attr("src"),
					    ImageDesc: $(data).find('img').attr("alt")
					});
            }
                ...
                this.video.show()
					.changeVideo({
					    nowIndex: index,
					    Url: $(data).find('img').attr("data-big-url")
					});
