    ...
    var Travellers = function () {//出游人
        ...
        Visitor.prototype = {
            ...
            bindEvent: function () {
                ...
                .on('focus', '[role="idCardType"]', function(){
                    $.each($(this).find('option'), function(i, k){
                        if( $(k).val() == 100 ){
                            $(k).remove();
                        }
                    });
                    idCardSel($($(this).find('option')[0]).val());
                })
            ...
            checkIdCard: function (str) {
                var role = this.role;
                // 稍后提供验证
                var laterCheckDom = role.idCardNo.get(0);
                if( $(laterCheckDom).prev().val() == 100 ){
                  this.showTip(cQuery(laterCheckDom), '请先选择正确的证件类型');
                  return false;
                };
