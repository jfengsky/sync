...
define(function(){
	return '{{#Flights}}\
                     {{#each FlightDetails}}\
                     <tr  data-key="{{../FlightNo}}{{../Sequence}}{{../TakeOffDepartDate}}{{../DepartTime}}{{../FlightDetails.0.Marked}}{{../Price}}" data-open="{{../FlightNo}}{{../Sequence}}{{../TakeOffDepartDate}}{{../DepartTime}}" data-color="{{../FlightNo}}{{../Sequence}}{{../TakeOffDepartDate}}{{../DepartTime}}" {{#border @index ../FlightDetails.length}}{{/border}}>\
                        {{#if ../FlightStep}}\
                       ...
                       {{#if Duration}}\
                       <tr class="no_border" data-key="{{../../FlightNo}}{{../../Sequence}}{{../../TakeOffDepartDate}}{{../../DepartTime}}{{../Marked}}{{../../Price}}" data-open="{{../../FlightNo}}{{../../Sequence}}{{../../TakeOffDepartDate}}{{../../DepartTime}}" data-color="{{../../FlightNo}}{{../../Sequence}}{{../../TakeOffDepartDate}}{{../../DepartTime}}"><td colspan="6"><div class="transfer_plane"><span><i></i>{{ArriveCityName}}（{{ArriveAirportName}}）中转&nbsp;{{Duration}}</span></div></td></tr>\
                       {{/if}}\
                    {{/each}}\
                      {{#if DirectFlightChannel}}\
                         <tr  data-open="{{../FlightNo}}{{../Sequence}}{{../TakeOffDepartDate}}{{../DepartTime}}" data-color="{{../FlightNo}}{{../Sequence}}{{../TakeOffDepartDate}}{{../DepartTime}}"><td colspan="8"><div class="luggage_limit_info"><i></i>请注意您购买机票的免费行李额度，该机票婴儿无法预定。</div></td></tr>\
                      {{/if}}\
                    {{/Flights}}'
})