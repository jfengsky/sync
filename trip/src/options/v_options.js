 ...
 define(function(require, exports) {
   var $ = require("jquery"),
      amountPrice = Math.ceil(GV.app.order.initData.price.Amount),
      options = {
       tpl: {
         /*产品模板*/
      ...
      /*模板注册方法*/
         ...
         var _dateDom = options.dom.dateDom,
             ...
             $(_optionsDom).delegate(_dateDom, "change", function() {
               ...
               var _type = $(this).find("option:selected").data("type").split("@"),
                   ...
                   _total = Math.ceil(-_portion.data("price") * _portion.data("select") + _select  * _ctrip),
                    /*显示单项总价*/
                   _result = parseFloat(_ctrip * _select).toFixed(2);
                   /*获取不同日期对应的不同产品的份数*/
                   ...
                   if (_total != 0) {
                    $(_surchargeDom).html(parseFloat(options.config.Surcharge += _total).toFixed(2));
                     options.tips.showDom(options.config.Surcharge, _surchargeLiDom);
                    } 
                   /*计算总价*/
                   $(_totalDom).html(Math.ceil(amountPrice += _total));
         ...
         $(_optionsDom).delegate(_portionDom, "change", function() {
           var _val = $(this).find("option:selected").val(),
               ...
               _dateSelect = _parents.find(_dateDom).find("option:selected");
                /*价差*/
               _totalPrice = Math.ceil((_val - _selected) * _price),
               /*显示单项总价*/
               _result = parseFloat(_price * _val).toFixed(2); 
               if (_price > 0) {
                   _count.html("+" + _result);
                } 
                 else if (_price < 0) {
                  ...
                  $(_surchargeDom).html(parseFloat(options.config.Surcharge += _totalPrice).toFixed(2));
                   options.tips.showDom(options.config.Surcharge, _surchargeLiDom);
                 if (_val > 0) {
                   _iDom.addClass(_classDom);
                 } 
               ...
               /*计算总价*/
               $(_totalDom).html(Math.ceil(amountPrice += _totalPrice));
       ...
       countPrice: function(pSurcharge) {
           var _price = options.config.initData.price,
               _chart = options.config.initData.ChatUrl,
             _wrapDom = options.dom.wrapDom,
             _totalDom = options.dom.totalDom,
             _priceData = {
                 Amount: Math.ceil(amountPrice += pSurcharge),                      /*总价*/
           ...
           $(_totalDom).html(Math.ceil(amountPrice));
       ...
       products: function() {
         var _productData = options.config.initData,
             _productUrl = "/booking/ajax/OptionsNew/OptionalResourceSearch.ashx",
             ...
             _query = {
               ...
               ReturnCount: 9999,
               ResourceRelations: _productData.OptResRelationsJson,
               isSuperOrder: _productData.isSuperOrder
             },
              /*依赖DOM*/
             _optionsDom = options.dom.optionsDom,
            ...
            /*错误跳转*/
             _error = function() {
                if(GV.app.order.initData.ClientSource == "Offline") window.location.href = "OrderInternale.aspx?" + _param;
                else window.location.href = "Order.aspx?" + _param;
             };
             /*加载前提示*/
       ...
       submit: function() {
         ...
         var _portionDom = options.dom.portionDom,
             ...
             /*URL参数需要带在接口上*/
             _param = window.location.search.substr(1),
             _url = "/booking/ajax/OptionsNew/BookOptionsV1.aspx?" + _param,
             /*组合数据将用户选择的产品传递到接口*/
             _group = {
