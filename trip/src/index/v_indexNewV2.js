...
define(function (require, exports, module) {
    ...
    var v_index = {
        ...
        tpl: {    //模板
			...
			hotLeft:'<div class="left_detail">\
						 {{> left}}\
						</div>',
			hotMid:'<div class="middle_detail">\
						{{> mid}}\
						</div>',
			hotRight:'<div class="right_detail">\
						{{> right}}\
						</div>',
			imgLeft:'{{#if  URL}}<a href="{{URL}}"  target="_blank" class="side_img_l"><img src="{{Image}}" alt=""></a>{{else}}<span><img src="{{Image}}" alt=""></span>{{/if}}',
			imgMid:'{{#if  URL}}<a href="{{URL}}"  target="_blank" class="side_img_m"><img src="{{Image}}" alt=""></a>{{else}}<span><img src="{{Image}}" alt=""></span>{{/if}}',
			imgRight:'{{#if  URL}}<a href="{{URL}}"  target="_blank" class="side_img_r"><img src="{{Image}}" alt=""></a>{{else}}<span><img src="{{Image}}" alt=""></span>{{/if}}',
			hotContent:	'{{#if AreaName}}\
							 {{#with AreaName}}\
								{{#if  IsStrong}}\
									<h3><a href="{{URL}}" target="_blank" class="orange">{{Text}}</a></h3>\
									{{else}}\
						...
						{{/if}}\
						{{#each  DestKey}}\
							<dl class="subother_list">\
								{{#if URL}}\
									{{#if IsStrong}}\
										<dt><a href="{{URL}}" target="_blank" class="orange">{{Text}}</a></dt>\
										{{else}}\
										<dt><a href="{{URL}}" target="_blank" >{{Text}}</a></dt>\
									{{/if}}\
								{{else}}\
									{{#if IsStrong}}\
										<dt class="orange">{{Text}}</dt>\
									{{else}}\
										<dt >{{Text}}</dt>\
									{{/if}}\
								{{/if}}\
								<dd>\
									{{#each  Childen}}\
										{{#if IsStrong}}\
											<a href="{{URL}}" target="_blank" class="orange">{{Text}}</a>\
										{{else}}\
         ...
         hotContent4: '{{#if AreaName}}\
			...
			hotContentNI:'{{#each  DestAreaLst}}\
						...
						{{/each}}',					
			hotSide:'<ul class="side_jmp_dest">\
						{{#each DestAreaLst}}\
							 <li>\
							{{#with AreaName}}\
								{{#if IsStrong}}\
        ...
        */
        hotDest: function () {
            var self = this;
            var jPanel = $('div.sidenav_destination');
            var jContainer = jPanel.find('.destination_list');
            ...
            return {
                ...
                */
                setDefault: function (oData) {
                    ...
                    var dataKey;
                   //循环出html模版
					for(var Li=0;Li<oData.length;Li++){
						// Handlebar 填充公用的模板数据
						Handlebars.registerPartial("left", self.tpl.hotContent);
						Handlebars.registerPartial("right", self.tpl.hotContent);
						Handlebars.registerPartial("mid", self.tpl.hotContent);
						var key=oData[Li].Key;
						//判断数据符合哪一种UI原型
						if(oData[Li].DestAreaLst){
						  if(oData[Li].DestAreaLst[0].DestKey){
							var  DestAreaLstL=oData[Li].DestAreaLst[0].DestKey.length;
                            var CategoryID=oData[Li].CategoryID;
							if(DestAreaLstL>0){
								if(oData[Li].DestAreaLst[0].DestKey[0].Childen){
									var oDataLength=oData[Li].DestAreaLst.length;
									//两张图片的UI
									if(CategoryID==8||CategoryID==13){
										var htmlLeft=Handlebars.compile(self.tpl.hotLeft)(oData[Li].DestAreaLst[0]);
										var htmlRight=Handlebars.compile(self.tpl.hotRight)(oData[Li].DestAreaLst[1]);
										var imgLeft=Handlebars.compile(self.tpl.imgLeft)(oData[Li].DestAreaLst[0]);
										var imgRight=Handlebars.compile(self.tpl.imgRight)(oData[Li].DestAreaLst[1]);
										var html='<div class="side_jmp jmp_pic_2" style="display:none;top:0px" >'+htmlLeft+htmlRight+imgLeft+imgRight+'</div>';
										jCurDom = $(html);
									...
									if(CategoryID==10){
										var htmlLeft=Handlebars.compile(self.tpl.hotLeft)(oData[Li].DestAreaLst[0]);
										var htmlMid=Handlebars.compile(self.tpl.hotMid)(oData[Li].DestAreaLst[1]);
										var htmlRight=Handlebars.compile(self.tpl.hotRight)(oData[Li].DestAreaLst[2]);
										var imgLeft=Handlebars.compile(self.tpl.imgLeft)(oData[Li].DestAreaLst[0]);
										var imgMid=Handlebars.compile(self.tpl.imgMid)(oData[Li].DestAreaLst[1]);
										var imgRight=Handlebars.compile(self.tpl.imgRight)(oData[Li].DestAreaLst[2]);
										var html='<div class="side_jmp jmp_pic_3" style="display:none;top:0px" >'+htmlLeft+htmlMid+htmlRight+imgLeft+imgMid+imgRight+'</div>';
										jCurDom = $(html);
										jTempWrap.append(jCurDom);
										jCurDom.data('data-key', key);
										// oSubitem存放浮出层对应键值表
                                    ...
                                    //4层的情况
                                    if(CategoryID==9){
                                        var htmlLeft1=Handlebars.compile(self.tpl.hotContent4)(oData[Li].DestAreaLst[0]);
                                        var htmlLeft2=Handlebars.compile(self.tpl.hotContent4)(oData[Li].DestAreaLst[2]);
                                        var htmlRight1=Handlebars.compile(self.tpl.hotContent4)(oData[Li].DestAreaLst[1]);
                                        var htmlRight2=Handlebars.compile(self.tpl.hotContent4)(oData[Li].DestAreaLst[3]);
                                        var imgLeft,imgRight;
                                        for(var imgLen=0;imgLen<4;imgLen++){
                                            if(oData[Li].DestAreaLst[imgLen].Image){
                                                imgLeft=Handlebars.compile(self.tpl.imgLeft)(oData[Li].DestAreaLst[imgLen]);
                                                for(var imgSec=imgLen+1;imgSec<(3-imgLen);imgSec++){
                                                    if(oData[Li].DestAreaLst[imgSec].Image){  
                                                             imgRight=Handlebars.compile(self.tpl.imgRight)(oData[Li].DestAreaLst[imgSec]);
                                                             break;
                                                     }
                                                }
                                                break;
                                        ...
                                        var html='<div class="side_jmp jmp_pic_2" style="display:none;top:0px" >'+'<div class="left_detail">'+htmlLeft1+htmlLeft2+'</div>'+'<div class="right_detail">'+htmlRight1+htmlRight2+'</div>'+imgLeft+imgRight+'</div>';
                                        jCurDom = $(html);
                                        jTempWrap.append(jCurDom);
                                        jCurDom.data('data-key', key);
                                        // oSubitem存放浮出层对应键值表
									...
									var imgT,
										imgUrl,
										html;
									for(var a=0;a<oDataLength;a++){
										if(oData[Li].DestAreaLst[a].Image){
											imgT=oData[Li].DestAreaLst[a].Image;
											imgUrl=oData[Li].DestAreaLst[a].URL;
											break;
										}
									}
									//一张图片的UI
								    if(CategoryID==6||CategoryID==11||CategoryID==12){
                                        if(imgUrl){
                                            html='<div class="side_jmp jmp_pic" style="display:none;top:0px" >'+Handlebars.compile(self.tpl.hotSide)({DestAreaLst:oData[Li].DestAreaLst})+'<div class="side_img_wrap"><a href="'+imgUrl+'" target="_blank"><img src="'+imgT+'" alt="" class="jmp_img"></a></div></div>';
                                        }else{
                                            html='<div class="side_jmp jmp_pic" style="display:none;top:0px" >'+Handlebars.compile(self.tpl.hotSide)({DestAreaLst:oData[Li].DestAreaLst})+'<div class="side_img_wrap"><span><img src="'+imgT+'" alt="" class="jmp_img"></span></div></div>';
                                          }
                                    }else if(CategoryID==7){
									//国内旅游无图UI
									Handlebars.registerPartial("left", self.tpl.hotContentNI);
									Handlebars.registerPartial("right", self.tpl.hotContentNI);
										var destLength=Math.ceil(oData[Li].DestAreaLst.length/2);
										var oDataL={
												DestAreaLst:[]
											},
											oDataR={
										...
										for(var Dj=0;Dj<oData[Li].DestAreaLst.length;Dj++){
											if(Dj<destLength){
												oDataL.DestAreaLst.push(oData[Li].DestAreaLst[Dj]);
											}else{
												oDataR.DestAreaLst.push(oData[Li].DestAreaLst[Dj]);
											}
										}
										var htmlLeft=Handlebars.compile(self.tpl.hotLeft)(oDataL);
										var htmlRight=Handlebars.compile(self.tpl.hotRight)(oDataR);
										var html='<div class="side_jmp jmp_nopic_2" style="display:none;top:0px" >'+htmlLeft+htmlRight+'</div>';
									}
									jCurDom = $(html);
									jTempWrap.append(jCurDom);
									jCurDom.data('data-key', key);
                ...
                */
                _setTop: function (curKey) {
                    ...
                    if(browser=="Microsoft Internet Explorer" && trim_Version=="MSIE7.0") {
                       containerTop=containerTop-1;
                     }
                    // 当前点point的绝对位置
                    var pointTop = jPoint.offset().top;
                    // 浮动层subitem的高度
                    //加上图片高度
                    if(jSubitem.css("padding-bottom")){
                        var padH=parseInt(jSubitem.css("padding-bottom").substr(0,(jSubitem.css("padding-bottom").length-2)));
                        var subitemHeight = jSubitem.height()+padH;
                    }else{
                ...
                */
                anchorAttach: function () {
					...
					//右边浮出层链接
                    var subLists, subAnchor1,subAnchor2,subTitleAnchor1,subTitleAnchor1_1,subTitleAnchor2,subImgAnchor1,subImgAnchor2;
                    ...
                    jSubitems.each(function (i) {
						...
						if($(this).find('>ul').length>0){
							subLists = $(this).find('>ul');
						}else{
							subLists = $(this).find('>div');
						}
                        subLists.each(function (j) {
                            anchorNum = 0;
							//h3链接
							if($(this).find('>h3 a')[1]){
								subTitleAnchor1 = $(this).find('>h3 a')[0];
								if (subTitleAnchor1 && !subTitleAnchor1.getAttribute("data-ctm")) {
									ctmStr = ["#ctm_ref=va_" + subChannel + "_s", $$.StartCity, "_" + subArea + "_" + settings.page + "_l", lineIndex + 1, "_" + (j + 1+j) + "_txt"].join("");
									subTitleAnchor1.setAttribute("data-ctm", ctmStr);
								}
								subTitleAnchor1_1 = $(this).find('>h3 a')[1];
								if (subTitleAnchor1_1 && !subTitleAnchor1_1.getAttribute("data-ctm")) {
									ctmStr = ["#ctm_ref=va_" + subChannel + "_s", $$.StartCity, "_" + subArea + "_" + settings.page + "_l", lineIndex + 1, "_" + (j + 1+1+j) + "_txt"].join("");
									subTitleAnchor1_1.setAttribute("data-ctm", ctmStr);
								}
							}else{
								subTitleAnchor1 = $(this).find('>h3 a')[0];
								if (subTitleAnchor1 && !subTitleAnchor1.getAttribute("data-ctm")) {
							...
							//dt链接
							subTitleAnchor2 = $(this).find('dt >a')[0];
                            if (subTitleAnchor2 && !subTitleAnchor2.getAttribute("data-ctm")) {
                                ctmStr = ["#ctm_ref=va_" + subChannel + "_s", $$.StartCity, "_" + subArea + "_" + settings.page + "_l", lineIndex + 1, "_" + (j + 1) + "_txt"].join("");
                                subTitleAnchor2.setAttribute("data-ctm", ctmStr);
                            }
							//dd链接
                            subAnchor1 = $(this).find('dd >a');
							subAnchor1.each(function (k) {
                                isIgnore = settings.ignore.name ? (!this.getAttribute(settings.ignore.name) || this.getAttribute(settings.ignore.name) == settings.ignore.value) : false;
                                // if the anchor sould be igonred, then add the anchorNum value to mark this ignored the anchor. 
                                // The next anchor will minus the anchorNum to get the correct number.
                                if (isIgnore) {
							...
							//ul 下的div链接
							subAnchor2 = $(this).find('div >a');
                            subAnchor2.each(function (n) {
                                isIgnore = settings.ignore.name ? (!this.getAttribute(settings.ignore.name) || this.getAttribute(settings.ignore.name) == settings.ignore.value) : false;
                                // if the anchor sould be igonred, then add the anchorNum value to mark this ignored the anchor. 
                                // The next anchor will minus the anchorNum to get the correct number.
                                if (isIgnore) {
                                ...
                                if (!this.getAttribute("data-ctm")) {
                                    ctmStr = ["#ctm_ref=va_" + subChannel + "_s", $$.StartCity, "_" + subArea + "_" + settings.page + "_l", lineIndex + 1, "_" + (j + 1) + "." + (n + 1 - anchorNum) + "_txt"].join("");
                                    this.setAttribute("data-ctm", ctmStr);
                                }
                            });
                        });
                        ...
                        // 右下角图片添加跟踪代码
                        subImgAnchor1 = $(this).find('>a');
						var imgNum=0;
						//2 3张图片
						subImgAnchor1.each(function (m) {
						if ($(this) && !$(this)[0].getAttribute("data-ctm")) {
								isIgnore = settings.ignore.name ? (!this.getAttribute(settings.ignore.name) || this.getAttribute(settings.ignore.name) == settings.ignore.value) : false;
								if (isIgnore) {
                                    imgNum++;
                                    return true;
                                ...
                                if (!this.getAttribute("data-ctm")) {
                                    ctmStr = ["#ctm_ref=va_" + subChannel + "_s", $$.StartCity, "_" + subAreaImg + "_" + settings.page + "_l", lineIndex + m, "_" + m + "_txt"].join("");
                                    this.setAttribute("data-ctm", ctmStr);
                                }
							}
						});
						//一张图片
						subImgAnchor2 = $(this).find('>div >a')[0];
                        if (subImgAnchor2 && !subImgAnchor2.getAttribute("data-ctm")) {
                            ctmStr = ["#ctm_ref=va_" + subChannel + "_s", $$.StartCity, "_" + subAreaImg + "_" + settings.page + "_l", lineIndex + 1, "_" + 1 + "_txt"].join("");
                            subImgAnchor2.setAttribute("data-ctm", ctmStr);
                        }
                    });
        ...
        */
        setHref: function () {
            var resetAnchors = $(".destination_list").find("a[data-url]");
